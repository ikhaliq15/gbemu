...
constexpr uint8_t upperByte(uint16_t n) noexcept { return static_cast<uint8_t>(n >> 8); }

constexpr uint8_t lowerByte(uint16_t n) noexcept { return static_cast<uint8_t>(n); }
...
constexpr uint16_t setLowerByte(uint16_t n, uint8_t b) noexcept { return static_cast<uint16_t>((n & 0xFF00u) | b); }
...
constexpr uint8_t swapNibbles(uint8_t n) noexcept { return static_cast<uint8_t>((n << 4) | (n >> 4)); }
...
template <std::unsigned_integral T> constexpr bool getBit(T val, uint8_t bit) noexcept { return (val >> bit) & T{1}; }
...
constexpr bool addHadCarry(uint16_t a, uint16_t b) noexcept { return static_cast<uint32_t>(a) + b > 0xFFFF; }
...
constexpr bool addHadHalfCarry(uint16_t a, uint16_t b) noexcept { return ((a & 0x0FFF) + (b & 0x0FFF)) > 0x0FFF; }
...
constexpr bool subHadHalfCarry(uint16_t a, uint16_t b) noexcept { return (a & 0x0FFF) < (b & 0x0FFF); }
