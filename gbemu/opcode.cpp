...
namespace gbemu {
    ...
    OPCode::OPCode(
        ...
        const std::vector<uint8_t> auxiliaryArguments,
    ...
    , command_(command)
    , mnemonic_(mnemonic)
    , auxiliaryArguments_(auxiliaryArguments)
    , operands_(operands)
    , bytes_(bytes)
    , cycles_(cycles)
    , additionalCycles_(additionalCycles)
    , jumpCondition_(jumpCondition)
    , flags_(flags)
    {}

    std::pair<std::map<uint8_t, OPCode>, std::map<uint8_t, OPCode>> OPCode::constructOpcodes(const std::string& opcodeDataFile)
    {
        ...
        for (const auto& opcode: opcodeData)
        {
            ...
            opcodeMap->insert({op, OPCode(op, command, mnemonic, auxiliaryArguments, operands, bytes, cycles, additionalCycles, jumpCondition, flags)});
        }

        return std::make_pair(opcodesMap, prefixedOpcodesMap);
    }

    uint8_t OPCode::opcode() const { return opcode_; }
    std::string OPCode::command() const { return command_; }
    std::string OPCode::mnemonic() const { return mnemonic_; }
    std::vector<uint8_t> OPCode::auxiliaryArguments() const { return auxiliaryArguments_; }
    std::vector<Operand> OPCode::operands() const { return operands_; }
    uint8_t OPCode::bytes() const { return bytes_; }
    uint8_t OPCode::cycles() const { return cycles_; }
    uint8_t OPCode::additionalCycles() const { return additionalCycles_; }
    OPCode::JumpCondition OPCode::jumpCondition() const { return jumpCondition_; }

    OPCode::Flags OPCode::flags() const { return flags_; }
    OPCode::Flag OPCode::flagZ() const { return flags_[0]; }
    OPCode::Flag OPCode::flagN() const { return flags_[1]; }
    OPCode::Flag OPCode::flagH() const { return flags_[2]; }
    OPCode::Flag OPCode::flagC() const { return flags_[3]; }
} // gbemu